...
records = 200000
n1 = np.zeros((200000, 1000))
n2 = np.zeros((200000, 1000))
n3 = np.zeros((200000, 1000))
n4 = np.zeros((20, 1000))


        
with open(r'\Users\mmishra\OneDrive - North Carolina State University\drs4_multiplexerr\8to1mult\dt5742\sdm_4to1\wave_0.dat', 'rb') as f:
    ct = 0 
    while(1):
...
with open(r'\Users\mmishra\OneDrive - North Carolina State University\drs4_multiplexerr\8to1mult\dt5742\sdm_4to1\wave_1.dat', 'rb') as f:
    ct = 0 
    while(1):
        b = np.fromfile(f, dtype=np.int32, count=6)
#        print('file header=',b)
...
with open(r'\Users\mmishra\OneDrive - North Carolina State University\drs4_multiplexerr\8to1mult\dt5742\sdm_4to1\wave_3.dat', 'rb') as f:
    ct = 0 
    while(1):
        b = np.fromfile(f, dtype=np.int32, count=6)
#        print('file header=',b)
        b1 = np.fromfile(f, dtype=np.float32, count=1024)
        n3[ct] = b1[15:1015] - np.mean(b1[15:95])
#        print('file header=',b)
        ct = ct + 1
        if ct > records-1:
            f.close()
...
with open(r'\Users\mmishra\OneDrive - North Carolina State University\drs4_multiplexerr\8to1mult\dt5742\sdm_4to1\wave_01.dat', 'rb') as f:
    ct = 0 
    while(1):
        b = np.fromfile(f, dtype=np.int32, count=6)
#        print('file header=',b)
...
#        print('file header=',b)
        ...
        if ct > records-1:
            ...
            break
records = 10
with open(r'\Users\mmishra\OneDrive - North Carolina State University\drs4_multiplexerr\8to1mult\dt5742\sdm_4to1\wave_11.dat', 'rb') as f:
    ct = 0 
    while(1):
        b = np.fromfile(f, dtype=np.int32, count=6)
#        print('file header=',b)
...
#        print('file header=',b)
        ...
        if ct > records-1:
            f.close()
            break
with open(r'\Users\mmishra\OneDrive - North Carolina State University\drs4_multiplexerr\8to1mult\dt5742\sdm_4to1\wave_31.dat', 'rb') as f:
    ct = 0 
    while(1):
        b = np.fromfile(f, dtype=np.int32, count=6)
#        print('file header=',b)
...
with open(r'\Users\mmishra\OneDrive - North Carolina State University\drs4_multiplexerr\8to1mult\dt5742\sdm_4to1\new\diode1\wave_0.dat', 'rb') as f:
    ct = 0 
    while(1):
        b = np.fromfile(f, dtype=np.int32, count=6)
#        print('file header=',b)
...
plt.plot(n1[203], label = 'SiPM signal')
plt.plot(n2[203], label = 'diode signal')
...
plt.plot(f0[0:500],np.abs(np.fft.fft(n4[2])[0:500]), label= 'diode signal')

plt.show()
plt.plot(f0[0:500],np.abs(np.fft.fft(n1[200])[0:500]), label= 'SiPM signal')
plt.plot(f0[0:500],np.abs(np.fft.fft(n2[200])[0:500]), label= 'diode signal')
#plt.plot(f0[0:500],np.abs(np.fft.fft(n1[2073])[0:500]), label= 'freq-encoded signal')
...
import copy
g_res50=[]
rres_50 = copy.deepcopy(n2)
rrec_50 = copy.deepcopy(n3)
t_ind = []
t_res50 = []
from scipy.interpolate import UnivariateSpline
max_res50 = np.zeros((len(rres_50),2))
rew = 0
for i in range(len(rrec_50)):#len(rec_50)
    max_arg1 = 100 + np.argmax(rres_50[i,100:900])
    abcissa1 = np.asarray([ind for ind in range(max_arg1-2, max_arg1+2)])
    ordinate1 = rres_50[i,abcissa1]
    spl1 = UnivariateSpline(abcissa1, ordinate1,k=3)
    xs1 = np.linspace(max_arg1-2, max_arg1+2,80)
...
#    max_res50[i,0] = np.max(rres_50[i,1,100:900])
    ...
    abcissa2 = np.asarray([ind for ind in range(max_arg2-2, max_arg2+2)])
    ordinate2 = rrec_50[i,abcissa2]
    spl2 = UnivariateSpline(abcissa2, ordinate2,k=3)
    xs2 = np.linspace(max_arg2-2, max_arg2+2,80)
...
plt.figure()
plt.hist2d(g_res50[:,0]/2**12*1000, g_res50[:,1]/2**12*1000, bins=[np.arange(5/2**12*1000, 1500/2**12*1000, 1/2**12*1000), np.arange(5/2**12*1000, 1500/2**12*1000, 1/2**12*1000)], cmin = 1)
...
plt.hist2d(g_res50[:,0]/2**12*1000, g_res50[:,1]/2**12*1000, bins=[np.arange(5/2**12*1000, 1500/2**12*1000, 1/2**12*1000), np.arange(5/2**12*1000, 1500/2**12*1000, 1/2**12*1000)], cmin = 1)
ax1.set_xlabel('SiPM pulse amplitude \n (mV)', fontsize=14)
ax1.set_ylabel('diode pulse amplitude \n (mV)', fontsize=14)
plt.colorbar()

# inset axes....
axins = ax1.inset_axes([0.1, 0.5, 0.47, 0.47])
axins.hist2d(g_res50[:,0]/2**12*1000, g_res50[:,1]/2**12*1000, bins=[np.arange(5/2**12*1000, 1500/2**12*1000, 1/2**12*1000), np.arange(5/2**12*1000, 1500/2**12*1000, 1/2**12*1000)], cmin = 1)
# sub region of the original image
axins.plot(xx, rational3_3(xx, *popt), label='fit', color = 'red', linewidth = 0.5)

x1, x2, y1, y2 = 2, 100, 2, 68
...
def diode_model1(x,a,b):
    return a*x**2 + b

def diode_model2(x,m,c):
    return m*x + c

amp_d = np.array(g_res50)
import pandas as pd
amp_d_df = pd.DataFrame(g_res50,columns = ['p1', 'd1'])
amp_d_df1 = amp_d_df.loc[lambda df: df['p1'] > 10*2**12/1000, :]

plt.figure()
plt.hist2d(amp_d_df1['p1']/2**12*1000, amp_d_df1['d1']/2**12*1000, bins=[np.arange(5/2**12*1000, 1500/2**12*1000, 1/2**12*1000), np.arange(5/2**12*1000, 1500/2**12*1000, 1/2**12*1000)], cmin = 1)
plt.xlabel('original peak - recovered peak \n (mV)')
plt.ylabel('recovered peak \n (mV)')
plt.colorbar()
plt.tight_layout()
...
t, y = variables('t, y')
a, b, c,d,e, t0 = parameters('a, b, c,d,e, t0')
...
fit = Fit(model, t=amp_d_df1['p1']/2**12*1000, y=amp_d_df1['d1']/2**12*1000, constraints=constraints)
...
plt.plot(g_res50[:,0], fit.model(t=g_res50[:,0], **fit_result.params).y)
plt.show()
...
def rational3_3(x, p0, p1, p2, p3, q1, q2, q3):
    return rational(x, [p0, p1, p2, p3], [q1, q2, q3])


from scipy.optimize import curve_fit
popt, pcov = curve_fit(rational3_3, g_res50[:,0]/2**12*1000, 
                       g_res50[:,1]/2**12*1000)
print(popt)
print(pcov)
...
xx = np.arange(2, 305, 0.5)

plt.plot(xx, rational3_3(xx, *popt), label='fit', color = 'red', linewidth = 0.5)
